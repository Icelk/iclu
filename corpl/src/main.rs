...
A good practise for the first line to only contain the comment string.")
        .arg(
            Arg::with_name("CONFIG")
                .help("Sets the config files to change. It is recommended to only use one config file per instance of this program, since the `-c` option overrides all unrecognised comment strings.")
                .required(true)
                .multiple(true),
        )
        .arg(
        ...
        .arg(
            Arg::with_name("c").help("Override comment string found in file. Can be used if the program failed to register it.")
                .short("c")
                .long("comment-string")
                .takes_value(true),
        );
    ...
    for file in matches.values_of("CONFIG").unwrap() {
        lib::process_file(
            Path::new(file),
            matches.value_of("c").map(str::as_bytes),
            &enabled,
        );
    }
}
